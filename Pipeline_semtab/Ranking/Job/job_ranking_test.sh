...
#SBATCH --output=logs/ranking_finetuning_%j.out
#SBATCH --error=logs/ranking_finetuning_%j.err
...
CONFIGS=(
    config/other_dataset/config_finetuning.txt
    config/other_dataset/config_finetuning_bigger_model.txt
)
...
if [ ${#OUTPUT_DIRS[@]} -gt 0 ]; then
    ...
    zip -r ranking_test_${SLURM_JOB_ID}.zip "${UNIQUE_DIRS[@]}"
    echo "Archive created: ranking_test_${SLURM_JOB_ID}.zip"
else
