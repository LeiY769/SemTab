...
CONFIGS=(
    config/finetuning/config_lora_3b.txt
    config/finetuning/config_lora_7b.txt
)
...
if [ ${#OUTPUT_DIRS[@]} -gt 0 ]; then
    ...
    zip -r ranking_finetuning_${SLURM_JOB_ID}.zip "${UNIQUE_DIRS[@]}"
    echo "Archive created: ranking_finetuning_${SLURM_JOB_ID}.zip"
else
