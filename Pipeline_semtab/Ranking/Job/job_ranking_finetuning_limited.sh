...
#SBATCH --output=logs/ranking_finetuning_limited_%j.out
#SBATCH --error=logs/ranking_finetuning_limited_%j.err
...
CONFIGS=(
    config/finetuning/config_lora_3b_limited.txt
    config/finetuning/config_lora_7b_limited.txt
    config/other_dataset/config_finetuning_limited.txt
    config/other_dataset/config_finetuning_limited_bigger_model.txt
)
...
if [ ${#OUTPUT_DIRS[@]} -gt 0 ]; then
    ...
    zip -r ranking_finetuning_limited_${SLURM_JOB_ID}.zip "${UNIQUE_DIRS[@]}"
    echo "Archive created: ranking_finetuning_limited_${SLURM_JOB_ID}.zip"
else
