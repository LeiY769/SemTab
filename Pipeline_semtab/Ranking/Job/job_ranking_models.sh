...
#SBATCH --output=logs/ranking_models_%j.out
#SBATCH --error=logs/ranking_models_%j.err
...
CONFIGS=(
    config/models/config_qwen25_0_5b.txt
    config/models/config_qwen25_1_5b.txt
    config/models/config_qwen25_7b.txt
    config/models/config_llama32_1b.txt
    config/models/config_llama32_3b.txt
    config/models/config_llama31_8b.txt
    config/models/config_gemma3_1b.txt
    config/models/config_gemma2_9b.txt
    config/models/config_glm4_9b.txt
)
...
if [ ${#OUTPUT_DIRS[@]} -gt 0 ]; then
    ...
    zip -r ranking_models_${SLURM_JOB_ID}.zip "${UNIQUE_DIRS[@]}"
    echo "Archive created: ranking_models_${SLURM_JOB_ID}.zip"
else
