...
#SBATCH --output=logs/ranking_prompts_%j.out
#SBATCH --error=logs/ranking_prompts_%j.err
...
CONFIGS=(
    config/prompts/config_oneshot.txt
    config/prompts/config_fewshot.txt
    config/prompts/config_cot.txt
    config/prompts/config_sc.txt
)
...
if [ ${#OUTPUT_DIRS[@]} -gt 0 ]; then
    ...
    zip -r ranking_prompts_${SLURM_JOB_ID}.zip "${UNIQUE_DIRS[@]}"
    echo "Archive created: ranking_prompts_${SLURM_JOB_ID}.zip"
else
