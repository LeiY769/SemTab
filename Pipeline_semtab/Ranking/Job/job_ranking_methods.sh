...
#SBATCH --output=logs/ranking_methods_%j.out
#SBATCH --error=logs/ranking_methods_%j.err
...
CONFIGS=(
    config/methods/config_noslm.txt
    config/methods/config_limited_slm.txt
    config/methods/config_full_slm.txt
    config/methods/config_slm_context.txt
)
...
if [ ${#OUTPUT_DIRS[@]} -gt 0 ]; then
    ...
    zip -r ranking_methods_${SLURM_JOB_ID}.zip "${UNIQUE_DIRS[@]}"
    echo "Archive created: ranking_methods_${SLURM_JOB_ID}.zip"
else
