...
#SBATCH --output=logs/ranking_test_noslm_%j.out
#SBATCH --error=logs/ranking_test_noslm_%j.err
...
CONFIGS=(
    config/other_dataset/config_noslm.txt
)
...
if [ ${#OUTPUT_DIRS[@]} -gt 0 ]; then
    ...
    zip -r ranking_test_noslm_${SLURM_JOB_ID}.zip "${UNIQUE_DIRS[@]}"
    echo "Archive created: ranking_test_noslm_${SLURM_JOB_ID}.zip"
else
