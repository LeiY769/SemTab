...
#SBATCH --output=logs/ranking_tablellama_%j.out
#SBATCH --error=logs/ranking_tablellama_%j.err
...
CONFIGS=(
    config/tablellama/config_default_prompts.txt
    config/tablellama/config_tablellama_prompts.txt
)
...
if [ ${#OUTPUT_DIRS[@]} -gt 0 ]; then
    ...
    zip -r ranking_tablellama_${SLURM_JOB_ID}.zip "${UNIQUE_DIRS[@]}"
    echo "Archive created: ranking_tablellama_${SLURM_JOB_ID}.zip"
else
