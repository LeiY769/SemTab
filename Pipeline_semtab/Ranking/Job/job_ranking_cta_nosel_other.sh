...
#SBATCH --output=logs/ranking_cta_nosel_other_%j.out
#SBATCH --error=logs/ranking_cta_nosel_other_%j.err
...
CONFIGS=(
    config/cta_nosel/other_dataset/config_limited_slm.txt
    config/cta_nosel/other_dataset/config_full_slm.txt
    config/cta_nosel/other_dataset/config_slm_context.txt
)
...
if [ ${#OUTPUT_DIRS[@]} -gt 0 ]; then
    ...
    zip -r ranking_cta_nosel_other_${SLURM_JOB_ID}.zip "${UNIQUE_DIRS[@]}"
    echo "Archive created: ranking_cta_nosel_other_${SLURM_JOB_ID}.zip"
else
