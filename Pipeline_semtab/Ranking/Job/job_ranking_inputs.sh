...
#SBATCH --output=logs/ranking_inputs_%j.out
#SBATCH --error=logs/ranking_inputs_%j.err
...
CONFIGS=(
    config/inputs/config_basic_prompt_10.txt
    config/inputs/config_nollm.txt
    config/inputs/config_glm_9b_30.txt
    config/inputs/config_glm_9b_5.txt
    config/inputs/config_qwen25_0_5b.txt
)
...
if [ ${#OUTPUT_DIRS[@]} -gt 0 ]; then
    ...
    zip -r ranking_inputs_${SLURM_JOB_ID}.zip "${UNIQUE_DIRS[@]}"
    echo "Archive created: ranking_inputs_${SLURM_JOB_ID}.zip"
else
