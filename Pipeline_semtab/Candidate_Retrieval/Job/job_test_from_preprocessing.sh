...
#SBATCH --output=logs/candidate%j.out
#SBATCH --error=logs/candidate%j.err
...
CONFIGS=(
    config/config_candidate_nollm.txt
    config/config_candidate_basic.txt
    config/config_candidate_no_few_shot.txt
    config/config_candidate_glm.txt
    config/config_candidate_llama.txt
    config/config_candidate_gemma.txt
    config/config_candidate_qwen2.txt
    config/config_candidate_qwen3.txt
)
...
if [ ${#OUTPUT_DIRS[@]} -gt 0 ]; then
    ...
    zip -r candidate_results_${SLURM_JOB_ID}.zip "${UNIQUE_DIRS[@]}"
    echo "Archive created: candidate_results_${SLURM_JOB_ID}.zip"
else
