...
#SBATCH --output=logs/candidate_finetuning_limit_%j.out
#SBATCH --error=logs/candidate_finetuning_limit_%j.err
...
CONFIGS=(
    config/config_lora_32_30.txt
    config/config_lora_32_50.txt
)
...
if [ ${#OUTPUT_DIRS[@]} -gt 0 ]; then
    ...
    zip -r candidate_finetuning_limit_${SLURM_JOB_ID}.zip "${UNIQUE_DIRS[@]}"
    echo "Archive created: candidate_finetuning_limit_${SLURM_JOB_ID}.zip"
else
