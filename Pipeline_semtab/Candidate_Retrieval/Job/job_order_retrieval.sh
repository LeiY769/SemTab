...
#SBATCH --output=logs/candidate_order_%j.out
#SBATCH --error=logs/candidate_order_%j.err
...
CONFIGS=(
    config/config_fuzzy_direct_valid.txt
    config/config_fuzzy_direct_training.txt
)
...
if [ ${#OUTPUT_DIRS[@]} -gt 0 ]; then
    ...
    zip -r candidate_order_${SLURM_JOB_ID}.zip "${UNIQUE_DIRS[@]}"
    echo "Archive created: candidate_order_${SLURM_JOB_ID}.zip"
else
