...
#SBATCH --output=logs/candidate_finetuning_%j.out
#SBATCH --error=logs/candidate_finetuning_%j.err
...
CONFIGS=(
    config/config_lora_32.txt
    config/config_lora.txt
    config/config_qlora.txt
)
...
if [ ${#OUTPUT_DIRS[@]} -gt 0 ]; then
    ...
    zip -r candidate_lora_qlora_${SLURM_JOB_ID}.zip "${UNIQUE_DIRS[@]}"
    echo "Archive created: candidate_lora_qlora_${SLURM_JOB_ID}.zip"
else
