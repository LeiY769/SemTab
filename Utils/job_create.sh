...
for cfg in "${CONFIGS[@]}"; do
    ...
    python -u dataset_build_ranking.py "$cfg"
    status=$?
    if [ $status -ne 0 ]; then
...
if [ ${#OUTPUT_DIRS[@]} -gt 0 ]; then
    ...
    zip -r dataset_${SLURM_JOB_ID}.zip "${UNIQUE_DIRS[@]}"
    echo "Archive created: dataset_${SLURM_JOB_ID}.zip"
else
